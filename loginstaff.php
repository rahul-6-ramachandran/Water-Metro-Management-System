...
if(isset($_POST['submit'])){
    ...
    $stpass=$_POST['stpd'];
    $res=mysqli_query($conn,"SELECT * FROM registration WHERE email_address = '$stEmail' AND password = '$stpass'");
    ...
    if($count==1){
        ...
        if($acct_type==2){
            ...
            $_SESSION["stId"]=$row["user_id"];
            $_SESSION["stnamE"]=$row["name"];
            $_SESSION["stPh_nO"]=$row["phone_no"];
            $_SESSION["stEmaiL"]=$row["email_address"];
        }
        else{
            ...
            echo '<script>location.href="Staffacc.php"</script>';
    }
    else{
    ...
    <div class="center" style="background-color:(rgb(162, 163, 164));">
        ...
        <form method="post">
            <div class="txt_field form-text">
                <input type="Email" name="stEmail" required>
                <span></span>
                <label>Email</label>
            </div>
            <div class="txt_field form-text">
                <input type="password" name="stpd" required>
                <span></span>
